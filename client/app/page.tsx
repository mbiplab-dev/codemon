...
function Room({ children }: { children: React.ReactNode }) {
  // Generate a unique room ID - in production, this might be based on project ID
  const roomId = `code-editor-${window.location.pathname.replace(/\//g, '-') || 'main'}`;
    ...
    >
      <ClientSideSuspense fallback={
        <div className="h-screen w-screen bg-black flex items-center justify-center">
          <div className="text-white">Loading collaborative features...</div>
        </div>
      }>
        {children}
      </ClientSideSuspense>
  ...
  const handleFileClick = async (file: FileType) => {
    try {
      ...
      setOpenFiles((prev) => prev.map(f => 
        f.path === file.path ? fileWithContent : f
      ));
